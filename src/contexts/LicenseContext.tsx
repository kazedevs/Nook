...
export function LicenseProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const resp = await invoke<LicenseCheckResponse>(
  ...
  const activate = useCallback(
    async (key: string): Promise<LicenseCheckResponse> => {
      const resp = await invoke<LicenseCheckResponse>("activate_license", {
        licenseKey: key,
      });
  ...
  const isLicensed =
    status?.status === "active" || status?.status === "active_offline";
